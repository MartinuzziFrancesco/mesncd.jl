...
DelimitedFiles, ProgressBars, JSON
...
shift = 300
train_lenghts = reverse([6500, 6000, 5500, 5000, 4500, 4000, 3500, 3000, 2500, 2000])
#train_len = 7000
predict_len = 2500
n_ic = 20
...
reservoir_inits = [delay_line, delay_line_backward, simple_cycle, double_cycle,
    selfloop_cycle, selfloop_feedback_cycle, selfloop_delayline_backward, selfloop_forward_connection,
    forward_connection]


for csystem in chaos_systems
    for train_len in train_lenghts
    #for init in reservoir_inits
        ...
        init_name = string(init)
        pop!(esn_params, :init_reservoir)
        open("results/min_esn/esn_results_$init_name$system_name$train_len.csv", "w") do io
            writedlm(io, esn_results, ',')
        end

        open("results/min_esn/params/esn_params_$init_name$system_name$train_len.json", "w") do io
